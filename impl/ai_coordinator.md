...
non-empty for MAKE_OFFER and REQUEST_ITEMS, that the acting villager has sufficient
inventory for MAKE_OFFER items (INVR-60), and that ACCEPT is only returned when the other
party's last recorded action is MAKE_OFFER (BHVR-63) — failing any check is a parse failure.
...
The data AI Coordinator reads from Memory System for one ordered pair `(speaker, subject)`
when assembling a relationship-update prompt. Grouping these two fields prevents the caller
from accidentally passing mismatched data extracted for different pairs.

```python
@dataclass
class RelationshipRecord:
    description: str        # ≤128 tokens; speaker's current overall view of subject
    impressions: list[str]  # up to 3 most recent; each ≤32 tokens; chronological order
```
...
Return type for all `assemble_*` functions in `prompts.py`. Named fields prevent callers
from confusing segments and breakpoints when passing to `LLMClient.complete()`.

```python
@dataclass
class PromptPackage:
    segments: list[PromptSegment]
    breakpoints: list[int]   # cache_breakpoint_indices for LLMClient
```
...
Cache breakpoints (the `breakpoints` field of `PromptPackage`) are placed at the end of
segment 4 (after other-character bios + relationships) and the end of segment 5 (after
memories). These mark the longest stable prefixes likely to be shared across successive
calls for the same villager.
...
contract. Contains all result structs, enums, PromptPackage, RelationshipRecord, and
ParseFailureLog.
...
Imports: llm_client.types (PromptSegment), action_system.types (ActionType),
game_types (ItemType, GameTime).
```
...
each takes fully-typed domain objects and returns a PromptPackage.
...
ConversationSnapshot, TradeSnapshot, ComputedStats).
```
...
to all subsystems it reads from. Each public method assembles a PromptPackage, builds
a ParseContext, and delegates to _call. _call handles LLM invocation and the retry-once
crash sequence shared by every call type.
...
that `items` is non-empty for MAKE_OFFER/REQUEST_ITEMS, that the villager holds
sufficient inventory for any MAKE_OFFER items, and that ACCEPT is only returned when
the other party's last recorded action was MAKE_OFFER (BHVR-63).
...
**`RelationshipRecord`** — The two fields AI Coordinator reads from Memory System when
assembling a relationship-update prompt: the speaker's current description of the
subject plus their 3 most recent impressions. Grouped to prevent callers from passing
mismatched fields extracted for different ordered pairs.

**`PromptPackage`** — Named two-field container returned by every `assemble_*`
function. Eliminates positional ambiguity when passing assembled segments and
breakpoints to `LLMClient.complete()`.
...
class ParseContext:
    ...
    game_time: GameTime          # game-minutes from epoch at call time
...
`prompt` field is the same segment list from `PromptPackage.segments` passed to
`LLMClient.complete()`.
...
All functions return `PromptPackage`. No LLM calls, no mutable state.
...
def assemble_conversation_turn(
    ...
    session history. `computed_stats` and `inventory_items` are included so the
    villager can reference their live condition (hunger, thirst, holdings) when
    deciding what to say or offer."""
```
...
def assemble_relationship_update(
    ...
    (speaker, subject). `relationship` supplies the speaker's current description
    of the subject and their 3 most recent impressions, giving the LLM context
    for whether their opinion has changed. Speaker precedes subject in the parameter
    list — reversing them is a semantic error."""
```
...
def parse_trade_turn(
    ...
    MAKE_OFFER/REQUEST_ITEMS, if MAKE_OFFER items exceed the villager's inventory
    (INVR-60), or if ACCEPT is returned when `last_other_action` is not MAKE_OFFER
    (BHVR-63). `last_other_action` is None when no offer has been made yet."""
```
...
Every public method assembles a `PromptPackage`, builds a `ParseContext`, and delegates
to `_call`. `_call` owns the retry-once crash sequence: on `ParseError` it retries once
with the same prompt, writes a `ParseFailureLog` entry for each failure, and crashes
...
def _call(
    ...
    """Invoke LLM with `package`, parse result with `parse_fn`. Retries once on
    ParseError; crashes on second failure. Writes ParseFailureLog before each raise."""
```

```python
def select_action(self, villager_id: str, game_time: GameTime) -> ActionSelectionResult:
...
def get_relationship_update(
    ...
    Reads the existing RelationshipRecord from `memory_system` for the ordered pair
    (speaker, subject) to populate the prompt."""
```
