...
→ STYLE: `_handle_action_complete` will be extremely complex — it sequences decay, two kinds of threshold crossings (death vs. force-sleep), prior-action completion, two separate memory-compaction triggers, safety recalculation, AI action selection, conversation detection + participant rescheduling, and fire-state sync. That is 8–10 distinct concerns in one function. It should be decomposed into named helpers (e.g., `_apply_thresholds`, `_maybe_compact_memory`, `_dispatch_chosen_action`, `_handle_conversation_result`) so cyclomatic complexity stays manageable and each concern is testable in isolation.

→ STYLE: `ScheduledEvent` uses optional payload fields (`villager_id`, `carcass_id`) where only one is populated per event type and the caller must know which to read based on `event_type`. This is an implicit tagged-union with no type-system enforcement. Prefer explicit subtypes — `ActionCompleteEvent(villager_id)`, `CarcassRotEvent(carcass_id)`, and marker classes for the three global events — and dispatch via `match` / `isinstance`. Eliminates the need for callers to know which optional field is populated.

→ STYLE: `villager_id: str` is used as a bare string across every subsystem boundary (Action System, AI Coordinator, Memory System, Conversation System, Villager State). A `VillagerId` newtype (or at minimum a `TypeAlias`) would catch the class of bugs where any `str` is silently accepted — including passing a `carcass_id` string, a character name, or a log key in the wrong position.

→ STYLE: `_sync_fire_event` must be called by the caller "after any action that may modify fire state" — this is a manual invariant with no compile-time enforcement. Any action handler that touches fuel or the fire flag but omits this call silently desyncs the extinction event. Consider instead having World State's fire-mutating setters return a "fire state changed" flag, and calling `_sync_fire_event` centrally in `run()` after every event dispatch rather than per-handler.

→ STYLE: The conversation-action branch inside `_handle_action_complete` (detect "Talk to someone", call `run_conversation`, iterate all participants to cancel and reschedule their `ACTION_COMPLETE` events) is a large block of special-case logic embedded in an already-complex function. Extract it as `_handle_conversation_action(initiator_id)` so the main handler stays a flat dispatcher and the participant-rescheduling logic is independently readable.

→ STYLE: `AutobalanceMultipliers.adjust()` accepts pre-aggregated floats (`avg_satiation_pct`, `avg_hydration_pct`, `avg_food_safety_days`) that `_handle_midnight` must compute by iterating villager states. The aggregation formula (especially `avg_food_safety_days`, which mirrors the safety score formula in Villager State) is now duplicated across two places that can drift independently. Either push the aggregation into `adjust()` (pass in the raw per-villager values as lists) or extract a standalone `compute_midnight_aggregates(villager_states, world_state) -> MidnightAggregates` function that both `_handle_midnight` and `adjust()` share.
