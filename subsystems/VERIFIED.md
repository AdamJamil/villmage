...
### Responsibilities
- Define the core entities and constants for items, weights, storage, water, fire capacity, fire fuel-unit values, dirtiness contributors, resting spots, and other persistent world objects.
- Track mutable ownership and location state for inventory, base storage, base-only water supply, fire on/off state plus remaining placed fuel, claimed resting spots, carcass age/rot deadlines, in-progress crafting, paused work, active sleep records, and other durable world records.
...
- Advance all time-based world processes at the correct boundaries, including passive stat drains, fire burn-down, carcass rot expiry, conversation turn time, sleep progression, midnight/day-end triggers, and checkpoint cadence.
- Own sequencing and lifecycle only. It does not decide action legality, survival formulas, relationship text, checkpoint storage formats, or replay UI behavior; it invokes the subsystems that own those rules.
...
### Dependencies
- `World State Model`: Reads who is busy, awake, alive, and located where, and writes top-level scheduling outcomes such as task start, task completion, interruption, resume, sleep records, and expiry/removal of timed world objects.
...
- `Survival State`: Advances passive drains, recomputes derived statuses after world changes, and asks for threshold outcomes such as collapse, forced sleep, death, and remedial warnings.
...
- Own action-local rules: argument ranges, visibility, exact prompt wording for action entries, item transformations, fuel-consumption order, recipe progression, exploration stop conditions, and other per-action behaviors.
...
- Own long-running action progression records for actions that can span multiple turns, especially partial crafting, and return enough structured execution data for the scheduler to resume or complete them later.
...
- `World State Model`: Reads inventories, base resources, base water, fire status, placed objects, carcass timers, and in-progress work, then writes the resulting item/location/task mutations when an action resolves.
- `Survival State`: Requests work-speed modifiers for displayed times, applies action-driven resource/stat changes such as calories, hydration, wakefulness, cleanliness, social effects, and sleep restoration, and relies on survival to interpret the resulting thresholds.
...
- Own sleep restoration math, including segmented overnight restoration when bed/fire conditions change during the same sleep interval and the corresponding feedback text derived from those segments.
...
- Resolve conversation-turn protocol exactly as a social state machine: collect candidate responses, choose the next actor by the specified priority order and recency tiebreak, discard non-winning turn inputs except concurrent leaves, append visible content to each participant's seen log, and stop after one hour or when only one participant remains.
- Run trading as a conversation-scoped interaction with its own turn rules, offer/request lifecycle, acceptance constraints, six-turn cancellation rule, zero-time trade turns, and inventory-transfer legality checks.
- Own the live social session state machines, including whose turn it is, who has joined, what each participant has seen, current offers, session transcripts, and timeout/cancellation rules.
...
- Enforce the structured-output contracts for each prompt family, including action selection, optional thought capture, join responses, conversation actions with required `resp` fields, trade actions with offer/request payloads, feeling scores, and relationship-update payloads.
...
- `Survival State`: Reads the current status descriptions, work-impacting interpretations, highest-salience subcomponent feedback, segmented sleep feedback, and remedial-warning override signals to decide what state feedback to include.
