...
- Expose profession-based capability flags that other subsystems use to decide what actions or recipes are even conceptually available, but not the runtime legality checks for any specific action.
...
### Responsibilities
- Define the core entities and constants for items, weights, storage, water, fire capacity, dirtiness sources, resting spots, and other persistent world objects.
- Track mutable ownership and location state for inventory, base storage, water supply, fire fuel state, placed resting spots, carcass timers, in-progress crafting, paused work, and other durable world records.
- Organize shared state into explicit domains rather than one undifferentiated store. At minimum this includes world resources and objects, actor task/execution records, and other non-social durable records needed across the simulation.
- Own where data lives for the shared simulation. This includes durable facts such as item counts, object placement, and task progress, but not formulas for how those facts affect health, mood, legality, or balance.
...
This subsystem advances time and coordinates the order in which the rest of the game runs. It is a thin scheduler that decides what is due next, dispatches to the subsystem that owns that transition, and commits results in a consistent order.

### Responsibilities
- Decide which villmager or live social session is due next, when prompts are issued, how action completion advances the simulation clock, and when conversation flow temporarily takes over from normal single-actor turns.
- Own sequencing and lifecycle only. It does not decide action legality, survival formulas, relationship text, checkpoint cadence rules, or replay formats; it invokes the subsystems that own those rules.
- Advance the clock, dispatch elapsed time or action/session completions to the owning subsystem, and apply the returned mutations and events in a consistent order.
- Maintain the small shared event vocabulary that other subsystems consume, such as action started, action completed, conversation started, conversation turn appended, conversation ended, sleep began, sleep ended, and villmager died.

### Dependencies
- `World State Model`: Reads who is busy, awake, alive, and located where, and writes top-level scheduling outcomes such as task start, task completion, interruption, and resumed work.
...
- `Social Interaction`: Hands off control when a conversation or trade starts, then resumes normal scheduling when the social flow ends.
- `Survival State`: Advances passive drains, recomputes derived statuses after world changes, and asks for threshold outcomes such as collapse, death, and remedial warnings.
- `Memory And Relationships`: Triggers thought capture and memory-compaction moments on the required cadence.
- `Prompt And Model Interface`: Uses it as the only path for action-turn, join-turn, and other model-facing prompt/response exchanges.
...
### Responsibilities
- Build the currently available action list for a villmager based on location, inventory, profession, world state, and blocking conditions such as encumbrance.
- Resolve the full catalog of non-social actions, including resource handling, exploration, fire tending, crafting, cooking, sleeping, resting, washing, and other survival work.
...
- Own action-local rules: argument ranges, visibility, legality, item transformations, fuel-consumption order, recipe progression, exploration stop conditions, and other per-action behaviors.
- Keep persistent facts in the world model and bodily or mood effects in survival state. The action system decides that butchering costs cleanliness and calories, for example, but survival owns the cleanliness and calorie ledgers themselves.
...
- `World State Model`: Reads inventories, base resources, fire status, placed objects, and in-progress work, then writes the resulting item/location/task mutations when an action resolves.
- `Survival State`: Requests work-speed modifiers for displayed times, applies action-driven resource/stat changes such as calories, hydration, wakefulness, cleanliness, and mood effects, and relies on survival to interpret the resulting thresholds.
...
- Own the formulas and thresholds for those values, including passive drains, sleep restoration, work-speed modifiers, death, forced sleep, and remedial warnings.
- Produce the derived metrics and prompt-facing interpretations built from those ledgers, including status descriptions and explanation snippets.
- Recalculate state from shared world facts where needed, including safety from stockpiles and mood/health explanations from their subcomponents.
...
### Dependencies
- `World State Model`: Reads canonical world facts such as inventories, stockpiles, firewood, dirtiness contributors, sleeping setup, and alive/dead membership, and writes only the survival-owned stat fields that change over time.
...
### Responsibilities
- Start, run, and end conversations, including join prompts, pause-and-resume behavior for interrupted work, turn sequencing, and elapsed conversation time.
- Run trading as a conversation-scoped interaction with its own turn rules, offer lifecycle, acceptance rules, and inventory transfer constraints.
- Own the live social session state machines, including whose turn it is, what each participant has seen, current offers, session transcripts, and timeout/cancellation rules.
- Emit compact session outcome records that downstream systems can consume.
- Leave long-term social interpretation to memory/relationships and direct stat math to survival. This subsystem decides that a conversation ended and who participated; other subsystems own what lasting relationship text or social score changes are written.
...
- Maintain relationship descriptions and recent impressions for every ordered villmager pair, updating them from social outcomes.
- Compact raw experience into short-, medium-, and long-term memory on the required cadence, with aggressive summarization to keep prompt context bounded.
- Own only private cognitive artifacts: logs, thoughts, memories, and relationship text. It does not own the authoritative action/event timeline, conversation control flow, or prompt transport.
...
### Responsibilities
- Construct villmager prompts in the required stable order, combining canon, relationships, memories, local world state, status descriptions, timestamp, and legal actions.
- Build the smaller specialized prompts used for conversation turns, social reflection, relationship updates, thought capture, and memory compaction.
- Own prompt templates, field ordering, serialization, model invocation, and response validation only. It does not decide which facts are true or which actions are legal; it renders caller-owned data into the required contract.
...
- `Action System`: Requests the current legal action list, argument schemas, displayed times, and action-specific wording for normal action-choice prompts.
- `Survival State`: Reads the current status descriptions, work-impacting interpretations, highest-salience subcomponent feedback, and remedial-warning override signals to decide what state feedback to include.
...
- Own storage format, replay reconstruction, checkpoint materialization, and the HTML/CSS/JavaScript observability surface.
